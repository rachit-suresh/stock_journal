...
export const TradeCard = ({ trade, currentPrice, onClose }: TradeCardProps) => {
  ...
  const isProfitable = currentPrice 
    ? (trade.direction === 'bullish' ? currentPrice > trade.entryPrice : currentPrice < trade.entryPrice)
    : false;
    ...
    <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <p className="text-lg font-semibold text-gray-900">${trade.entryPrice.toFixed(2)}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Stop Loss</p>
          <p className="text-lg font-semibold text-red-600">${trade.stopLoss.toFixed(2)}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Size</p>
          <p className="text-lg font-semibold text-gray-900">{trade.size} shares</p>
        </div>
        ...
        {trade.exitPrice && (
          <div>
            ...
            <p className="text-lg font-semibold text-gray-900">${trade.exitPrice.toFixed(2)}</p>
          </div>
        )}
      </div>

      {trade.status === 'open' && currentPrice && (
        <div className={`p-4 rounded-lg mb-4 ${isProfitable ? 'bg-green-50' : 'bg-red-50'}`}>
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-700">Unrealized P&L</span>
      ...
      {trade.result_pnl !== undefined && trade.status === 'closed' && (
        <div className={`p-4 rounded-lg mb-4 ${isProfit ? 'bg-green-50' : 'bg-red-50'}`}>
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-700">Realized P&L</span>
            <span className={`text-xl font-bold flex items-center ${
      ...
      <div className="flex items-center text-sm text-gray-500 mt-4">
        ...
        {trade.exitDate && (
          <span className="ml-4">Exit: {new Date(trade.exitDate).toLocaleDateString()}</span>
        )}
      </div>
